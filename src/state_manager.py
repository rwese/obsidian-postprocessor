...
class StatelessStateManager:
        ...
        try:
            ...
            if not frontmatter:
                ...
                if self._has_malformed_frontmatter(note_path):
                    self._log_frontmatter_error(f"Frontmatter parsing error in {note_path}: malformed YAML")
                return []
            
            processed = frontmatter.get('processed_recordings', [])
            if not isinstance(processed, list):
                logger.warning(f"Invalid processed_recordings format in {note_path}, expected list")
                return []
            ...
            if "ParserError" in str(e) or "while parsing" in str(e):
                self._log_frontmatter_error(f"Frontmatter parsing error in {note_path}: {e}")
                return []
            else:
                logger.error(f"Error getting processed recordings from {note_path}: {e}")
                return []
        ...
        try:
            ...
            try:
                ...
                if frontmatter is None:
                    ...
                    self._log_frontmatter_error(f"Frontmatter is not a dictionary, got {type(frontmatter)}, treating as empty")
                    frontmatter = {}
    ...
    def _find_note_file_in_vault(self, note_path: str) -> Optional[Path]:
        ...
        try:
            ...
            for pattern in [f"{note_name}.md", f"*/{note_name}.md", f"**/{note_name}.md"]:
    ...
    def get_broken_recordings(self, note_path: str) -> List[str]:
        ...
        try:
            ...
            if not isinstance(broken, list):
                logger.warning(f"Invalid broken_recordings format in {note_path}, expected list")
                return []
    ...
    def mark_recording_broken(self, note_path: str, recording_filename: str, error_message: str = None) -> bool:
        """
        ...
        try:
            ...
            logger.warning(f"Marked recording as broken: {recording_filename} in {note_path}" + 
